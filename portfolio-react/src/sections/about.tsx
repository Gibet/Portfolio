    ...
    <CustomSection id='about' pinned={pinned} lower={lower} ref={ref} zIndex={4}>
      <div className='flex flex-col items-center  w-5/6 h-full sm:py-20 py-6'>
        <Container variant='header' className='flex flex-col gap-2.5'>
          <h1 className="text-2xl font-bold terminal">A propos de moi</h1>
          <div className="flex flex-wrap gap-2">
            {AboutTabs.map((tab) => (
              <button
                key={tab}
                onClick={() => setCurrentTab(tab)}
                className={`px-3 py-1 ${currentTab === tab ? 'active' : ''}`}
              >
                {tab}
              </button>
            ))}
        ...
        <Container variant='body' className="mt-4 w-full h-full overflow-y-auto">
          {currentTab === 'Éducation' && (
              <ol className='sm:ml-6 sm:py-10 h-full relative py-10 overflow-y-auto'>
                <li className=''></li>
                <li className='mb-2 sm:ml-10 ml-6 flex items-center'>
                  <span className="absolute left-1.5 top-6 h-11/12 border-l chrono-line"></span>
                  <span className="absolute left-0 w-3.5 h-3.5 chrono-dot"></span>
                  <div>
                    <h2 className="text-xl font-semibold terminal">Bachelor Concepteur Développeur d'Applications (CDA)</h2>
                    <h4 className="text-md font-medium sm:ml-6">La Plateforme_ - 2022-2025</h4>
                    <ul className='list-disc sm:ml-10 ml-6'>
          ...
          {currentTab === 'Éxperience' && (
              <ol className='sm:ml-6 sm:py-10 h-full relative py-10 overflow-y-auto'>
                <li className='mb-2 sm:ml-10 ml-6 flex items-center'>
                  ...
                  <span className="absolute left-0 w-3.5 h-3.5 chrono-dot"></span>
                  <div>
                    <h2 className="text-xl font-semibold terminal">Développeur Web Junior (Alternance)</h2>
